...
# pirms 251730,22.04.2015, 2 6 23 24 34 ir citi html tokeni (https://www.latloto.lv/lv/arhivs/latloto/18)

#### HOWTO RUN ##################################################################################################
# Use Internet Download Manager to get all result files (20) from https://www.latloto.lv/lv/arhivs/latloto/1-20
# Save all files in folder "C:\Users\agrisk\Documents\Loto\Perl\RezultatuArhivs\5no35"
# edit $izlozu_skaits - cik izlozes apstradat. Taam jaabuut ieksaa nolasiitajos .htm failos
# run from cmd prompt "C:\Users\agrisk\Documents\Loto\Perl\5no35.pl"

#### EDIT PARAMETERS HERE ###############################################################, 1 - print#########################
my $skaitli_diapazons = 35;   		# 35, kopejais bumbinu skaits
my $skaitli_skaits = 5;				# 5, izveleto bumbinu skaits
my $papildskaitli_diapazons = 0;    # 0, papildskaitlu diapazons
my $papildsk_skaits = 1;			# 0, papildskaitlu skaits
my $izlozu_skaits = 100;			# cik pedejas izlozes apstradat
...
my $avg_sum = 90;  # average sum
my $lps = 68;  # lowest probable sum
my $hps = 112;  # highest probable sum
my $ls = 20;  # lowest sum (mod %10 must be zero)
my $hs = 150; # highest sum (mod %10 must be zero)
#################################################################################################################
...
my @array_partial = ();
my $book = ReadData ('../RezultatuArhivs/5no35/latloto.xls');
...
for my $r ( @array_partial )
{
	...
	for ( @$arr )
	{
		...
		$sum_drawing_100 += $sum_drawing if ($q < 100);
		$q++;
}
...
$strd_ = " " x (($tb_->colrange(6))[0] - length($str0) - length($stra) - length($strb) - length($strc)) . "SUM_DRAWING_10(20)(50)(100): $sum_drawing_10" . " ($sum_drawing_20)" . " ($sum_drawing_50)" . " ($sum_drawing_100)\n";
print $str0 . $stra_ . " " x (length($strb) + length($strc) - 10) . $strd_ . "\n";
...
print "Last draw results: " . (join ",", @{$array_partial[0]}[2..$skaitli_visi]);
print "\n\n";
my $percentage_system_drawings = 7;  # 7 drawings cheked - 5x7 = 35 - full cycle of all numbers in 7 drawings
